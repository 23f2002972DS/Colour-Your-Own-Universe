...
2. Run the first cell to install the required astronomy packages (!pip install astroquery astropy).
3. Execute the setup cells to load the Python tools and initialize the target library.
4. Run the randomizer cell to automatically select one of the 15 targets and a specific colormap.
5. The notebook will automatically download the 700x700 pixel image data, apply the brightness stretch, and plot the final visualization.
...
- Galaxies: Andromeda Galaxy (M31), Triangulum Galaxy (M33), Whirlpool Galaxy (M51), Bode's Galaxy (M81), Sombrero Galaxy (M104).
- Nebulae: Orion Nebula (M42), Lagoon Nebula (M8), Trifid Nebula (M20), Dumbbell Nebula (M27), Ring Nebula (M57).
- Star Clusters: Pleiades (M45), Hercules Globular Cluster (M13), Globular Cluster (M15).
- Supernova Remnants: Crab Nebula (M1), Veil Nebula (NGC 6992).
